...
PrintCrossLayerStats()
{
    ...
    const uint32_t totalSecondBuckets =
        GetWifiStatisticsState().coordinator.GetMaxExperimentDurationMs() > 0.0
            ? static_cast<uint32_t>(std::ceil(GetWifiStatisticsState().coordinator.GetMaxExperimentDurationMs() / 1000.0))
            : 0;
    const double experimentDurationSeconds = GetWifiStatisticsState().coordinator.GetMaxExperimentDurationMs() / 1000.0;
    ...
    for (const auto& [nodeId, label] : GetWifiStatisticsState().nodeLabels)
    {
        ...
        const auto nodeIt = GetWifiStatisticsState().nodeSeconds.find(nodeId);
        ...
        for (uint32_t second = 0; second < totalSecondBuckets; ++second)
        {
            ...
            if (nodeIt != GetWifiStatisticsState().nodeSeconds.end())
            {
