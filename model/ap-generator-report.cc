...
APGenerator::PrintPerSecondMetrics()
{
    ...
    NS_LOG_WARN("========== APGenerator per-second statistics ==========");
    ...
    for (const auto& [second, statistics] : m_metricsByAbsoluteSecond)
    {
        ...
        for (const auto& [stationAddress, stationBytes] : statistics.stationBytes)
        {
            const double stationThroughputMbps = static_cast<double>(stationBytes) * 8.0 / 1e6;
            const double bandwidthSharePercent =
                statistics.totalBytes > 0 ? static_cast<double>(stationBytes) /
                                                static_cast<double>(statistics.totalBytes) * 100.0
                                          : 0.0;

            NS_LOG_WARN("[Final per-second]   Station "
                        << InetSocketAddress::ConvertFrom(stationAddress).GetIpv4() << ": bytes="
                        << stationBytes << " throughput=" << stationThroughputMbps << " Mbps"
                        << " share=" << bandwidthSharePercent << "%");
        }
    }
